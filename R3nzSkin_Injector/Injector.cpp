	...
	if (LI_FN(K32EnumProcessModules)(hProcess, hMods, sizeof(hMods), &cbNeeded)) {
		for (auto i{ 0u }; i < (cbNeeded / sizeof(HMODULE)); ++i) {
			...
			if (LI_FN(K32GetModuleBaseNameW)(hProcess, hMods[i], szModName, sizeof(szModName) / sizeof(TCHAR))) {
				if (std::wcscmp(szModName, L"newSkin.dll") == 0) {
...
bool WINAPI Injector::inject(const std::uint32_t pid) noexcept
{
	...
	const auto dll_path{ std::wstring(current_dir) + L"\\newSkin.dll" };

	if (auto f{ std::ifstream(dll_path) }; !f.is_open()) {
		LI_FN(MessageBox)(nullptr, L"newSkin.dll file could not be found.\nTry reinstalling the cheat.", L"newSkin", MB_ICONERROR | MB_OK);
...
void Injector::run() noexcept
{
	...
	while (true) {
		...
		newSkinInjector::gameState = (league_processes.size() > 0) ? true : false;
		newSkinInjector::clientState = (league_client_processes.size() > 0) ? true : false;

		for (auto& pid : league_processes) {
			if (!Injector::isInjected(pid)) {
				newSkinInjector::cheatState = false;
				if (newSkinInjector::btnState) {
					std::this_thread::sleep_for(1.5s);
					if (Injector::inject(pid))
						newSkinInjector::cheatState = true;
					else
						newSkinInjector::cheatState = false;
				}
